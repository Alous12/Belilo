  ...
  template: `
    <div class="product-gallery" [class.large]="large()">
      @if (activeImage(); as image) {
        ...
        @else { <button type="button" class="gallery-photo-button" (click)="view.emit()" [attr.aria-label]="'Ver detalles de ' + product().name"><img class="gallery-photo" [src]="image" [alt]="product().name + ', foto ' + (activeIndex() + 1)"><span class="image-hint">Ver detalles ↗</span></button> }
      } @else { <span class="gallery-empty" aria-label="Sin imagen">✦</span> }
      @if (images().length > 1) {
        <button type="button" class="gallery-arrow previous" (click)="move(-1)" [attr.aria-label]="'Foto anterior de ' + product().name">‹</button>
        <button type="button" class="gallery-arrow next" (click)="move(1)" [attr.aria-label]="'Foto siguiente de ' + product().name">›</button>
